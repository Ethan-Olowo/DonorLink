...
class Database {
  ...
  Future<dynamic> getUser(String id) async {
    ...
    user = await docRef.get().then(
      (DocumentSnapshot doc) {
        ...
        if (data['type'] == 'donor') {
          user = Patient.fromFirestore(
            doc,
          );
          return user;
        } else if (data['type'] == 'organisation') {
          user = Hospital.fromFirestore(doc);
          return user;
    ...
    await db
      ...
      (querySnapshot) async {
        for (var docSnapshot in querySnapshot.docs) {
          ...
          if (type == 'donation')
            inters.add(
                Donation.fromFirestore(docSnapshot, user as Hospital, donor));
          if (type == 'appointment')
            inters.add(Appointment.fromFirestore(
                docSnapshot, user as Hospital, donor));
          if (type == 'rating')
            inters.add(
                Rating.fromFirestore(docSnapshot, user as Hospital, donor));
        }
      },
    );
    return inters;
  ...
  Future<bool> addRating(Rating rating) async {
    Hospital org = await rating.getOrg();
    ...
    return await db.runTransaction((transaction) async {
      ...
      final newAvg =
          (snapshot.get("rating") * (snapshot.get("ratings")) + rating.rating) /
              newRatings;
      transaction.update(orgDoc, {"ratings": newRatings, 'rating': newAvg});
