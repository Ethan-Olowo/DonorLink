...
class Donorcontroller extends Database {
  ...
  Future<List<Interaction>> getInteractions(User user, String type) async {
    ...
    await db.collection("Interactions").where("type", isEqualTo: type).where("donor", isEqualTo: user.id).get().then(
      (querySnapshot) async {
        for (var docSnapshot in querySnapshot.docs) {
          final data = docSnapshot.data() as Map<String, dynamic>;
          Organisation org = await getUser(data['org']);
          if(type=='donation')inters.add(Donation.fromFirestore(docSnapshot,org, user as Donor));    
          if(type=='appointment')inters.add(Appointment.fromFirestore(docSnapshot,org, user as Donor));    
          if(type=='rating')inters.add(Rating.fromFirestore(docSnapshot,org, user as Donor));    
        }
      },
    );
    return inters;
