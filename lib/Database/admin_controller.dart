...
class Admincontroller extends Database {
  ...
  Future<List<Interaction>> getInteractions(User user, String type) async {
    ...
    await db.collection("Interactions").where("type", isEqualTo: type).get().then(
      (querySnapshot) async {
        for (var docSnapshot in querySnapshot.docs) {
          final data = docSnapshot.data() as Map<String, dynamic>;
          Donor donor = await getUser(data['donor']);
          Organisation org = await getUser(data['org']);
          if(type=='donation')inters.add(Donation.fromFirestore(docSnapshot,org, donor));    
          if(type=='appointment')inters.add(Appointment.fromFirestore(docSnapshot,org, donor));    
          if(type=='rating')inters.add(Rating.fromFirestore(docSnapshot,org, donor));    
        }
      },
    );
    return inters;
    ...
    await db.collection("Interactions").get().then(
      (querySnapshot) async {
        for (var docSnapshot in querySnapshot.docs) {
          ...
          Donor donor = await getUser(data['donor']);
          Organisation org = await getUser(data['org']);
          if(data['type']=='donation')inters.add(Donation.fromFirestore(docSnapshot,org, donor));    
          if(data['type']=='appointment')inters.add(Appointment.fromFirestore(docSnapshot,org, donor));    
          if(data['type']=='rating')inters.add(Rating.fromFirestore(docSnapshot,org, donor));    
        }
      },
    );
    return inters;
  ...
  Future<List<User>> getAllUsers() async {
    ...
    (querySnapshot) {
      for (var docSnapshot in querySnapshot.docs) {
        final data = docSnapshot.data() as Map<String, dynamic>;
        print(data);
        if(data['type'] == 'donor')users.add(Donor.fromFirestore(docSnapshot,));
        if(data['type'] == 'organisation')users.add(Organisation.fromFirestore(docSnapshot,));
        if(data['type'] == 'reviewer')users.add(Reviewer.fromFirestore(docSnapshot,));  
